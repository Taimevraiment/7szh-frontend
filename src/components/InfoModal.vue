    ...
    <div class="info-outer-container" @click="cancel()">
        <div class="info-container" :class="{ 'mobile-font': isMobile }" v-if="isShow" @click.stop="">
            <div v-if="type == 5" @click.stop="showRule((info as Card).description, ...skillExplain.flat(2))">
                ...
                <div class="info-card-type sub"
                    v-for="(subtype, suidx) in (info as Card).subType.filter((v: number) => v > -1)" :key="suidx">
                    {{ CARD_SUBTYPE[subtype] }}
                </div>
                <div v-if="(info as Card).subType.includes(0)" class="info-card-type sub">
                    {{ WEAPON_TYPE[(info as Card).userType] }}
                </div>
                <div class="info-card-desc" v-for="(desc, didx) in (info as Card).descriptions" :key="didx"
                    v-html="desc"></div>
                <div class="info-card-explain" v-for="(expl, eidx) in skillExplain" :key="eidx"
            ...
            <div v-if="type < 5">
                ...
                <div v-if="type == 4" class="info-hero-tag">
                    ...
                    <span v-for="(local, lidx) in (info as Hero).local" :key="lidx">{{ HERO_LOCAL[local] }}</span>
                </div>
                <div class="info-hero-skill" v-for="(skill, sidx) in skills.filter((_, i) => type == 4 || i == type)"
                    :key="sidx">
                    <div class="info-hero-skill-title" @click.stop="showDesc(isShowSkill, sidx)">
                        <div style="display: flex;flex-direction: row;align-items: center;">
                            ...
                            <span class="info-skill-costs">
                                <div>{{ skill.name }}</div>
                                <div>
                                    <div class="skill-cost" v-for="(cost, cidx) in skill.cost.filter(c => c.val > 0)"
                                        :key="cidx"
                                        :style="{ color: cidx < 2 && skill.costChange[cidx] > 0 ? CHANGE_GOOD_COLOR : 'white' }">
                                        <img class="cost-img" :src="getDiceIcon(ELEMENT_ICON[cost.color])" />
                                        <span>{{ Math.max(cost.val - (cidx < 2 ? skill.costChange[cidx] : 0), 0)
                                                }}</span>
                                    </div>
                                </div>
                            </span>
                    ...
                    <div v-if="isShowSkill[sidx] && skillExplain[type < 4 ? type : sidx].length > 0"
                        ...
                        <div class="info-hero-skill-explain"
                            v-for="(expl, eidx) in skillExplain[type < 4 ? type : sidx]" :key="eidx">
                            <div v-for="(desc, didx) in expl" :key="didx" v-html="desc"></div>
                        </div>
                    </div>
                </div>
                <div v-if="type != 4">
                    <div class="info-equipment"
                        ...
                        <div class="equipment"
                            ...
                            <div v-if="isEquipment[slidx]"
                                ...
                                <div class="equipment-desc" v-for="(desc, didx) in (slot as Card).descriptions"
                                    :key="didx" v-html="desc">
                                </div>
                    ...
                    <div v-if="(info as Hero).inStatus.length > 0" class="info-status">
                        ...
                        <div v-for="(ist, idx) in (info as Hero).inStatus.filter((sts: Status) => !sts.type.includes(0))"
                                ...
                                <span class="status-title-left">
                                    <div class="status-icon">
                                        ...
                                        <div class="status-cnt"
                                            v-if="!ist.type.includes(10) && (ist.useCnt >= 0 || ist.roundCnt >= 0)">
                            ...
                            <div v-if="isInStatus[idx]"
                                @click.stop="showRule(ist.description, ...inStatusExplain[idx].flat(2))">
                                <div class="status-desc" v-for="(desc, didx) in ist.descriptions" :key="didx"
                                    v-html="desc">
                                </div>
                                <div v-if="inStatusExplain[idx].length > 0">
                                    <div class="info-hero-status-explain" v-for="(expl, eidx) in inStatusExplain[idx]"
                                        :key="eidx">
                                        <div v-for="(desc, didx) in expl" :key="didx" v-html="desc"></div>
                                    </div>
                                </div>
                    ...
                    <div v-if="(info as Hero).outStatus.length > 0" class="info-status">
                        ...
                        <div v-for="(ost, idx) in (info as Hero).outStatus.filter((sts: Status) => !sts.type.includes(0))"
                            ...
                            <div class="status-title" @click.stop="showDesc(isOutStatus, idx)">
                                <span class="status-title-left">
                                    <div class="status-icon">
                                        ...
                                        <div class="status-cnt"
                                            v-if="!ost.type.includes(10) && (ost.useCnt >= 0 || ost.roundCnt >= 0)">
                            ...
                            <div v-if="isOutStatus[idx]"
                                @click.stop="showRule(ost.description, ...outStatusExplain[idx].flat(2))">
                                <div class="status-desc" v-for="(desc, didx) in ost.descriptions" :key="didx"
                                    v-html="desc">
                                </div>
                                <div v-if="outStatusExplain[idx].length > 0">
                                    <div class="info-hero-status-explain" v-for="(expl, eidx) in outStatusExplain[idx]"
                                        :key="eidx">
                                        <div v-for="(desc, didx) in expl" :key="didx" v-html="desc"></div>
                                    </div>
                                </div>
            ...
            <div v-if="type == 6" @click.stop="showRule((info as Summonee).description)">
                ...
                <div class="summon-desc" v-for="(desc, didx) in (info as Summonee).descriptions" :key="didx"
                    v-html="desc"></div>
            </div>
        </div>
        <div class="info-container" :class="{ 'mobile-font': isMobile }" @click.stop=""
            ...
            <div v-if="(info as Hero).inStatus.length > 0" class="info-status">
                ...
                <div v-for="(ist, idx) in (info as Hero).inStatus.filter((sts: Status) => !sts.type.includes(0))"
                    ...
                    <div v-if="isInStatus[idx]"
                        @click.stop="showRule(ist.description, ...inStatusExplain[idx].flat(2))">
                        <div class="status-desc" v-for="(desc, didx) in ist.descriptions" :key="didx" v-html="desc">
                        </div>
                        <div v-if="inStatusExplain[idx].length > 0">
                            <div class="info-hero-status-explain" v-for="(expl, eidx) in inStatusExplain[idx]"
                                :key="eidx">
            ...
            <div v-if="(info as Hero).outStatus.length > 0" class="info-status">
                ...
                <div v-for="(ost, idx) in (info as Hero).outStatus.filter((sts: Status) => !sts.type.includes(0))"
                    ...
                    <div v-if="isOutStatus[idx]"
                        @click.stop="showRule(ost.description, ...outStatusExplain[idx].flat(2))">
                        <div class="status-desc" v-for="(desc, didx) in ost.descriptions" :key="didx" v-html="desc">
                        </div>
                        <div v-if="outStatusExplain[idx].length > 0">
                            <div class="info-hero-status-explain" v-for="(expl, eidx) in outStatusExplain[idx]"
                                :key="eidx">
...
import {
    ELEMENT, CARD_TYPE, CARD_SUBTYPE, ELEMENT_COLOR, SKILL_TYPE,
    CHANGE_GOOD_COLOR, WEAPON_TYPE, HERO_LOCAL, ELEMENT_ICON,
    STATUS_BG_COLOR, SKILL_TYPE_ABBR, RULE_EXPLAIN, CARD_SUBTYPE_URL, WEAPON_TYPE_URL,
} from '@/data/constant';
...
const wrapedIcon = (idx: number, isDice = false) => {
    ...
    return `<img style="width:18px;transform:translateY(20%);" src="${getPngIcon(ELEMENT_ICON[idx] + `${isDice ? '-dice-bg' : ''}`)}"/>`;
}
const wrapDesc = (desc: string, obj?: Skill | Status | Summonee): string => {
    let res = desc.slice()
        ...
        .replace(/(?<!\\)(｢)(.*?)(｣)/g, (_, prefix, word, suffix) => {
            ...
            if (sbtpIdx > -1 && !!CARD_SUBTYPE_URL[sbtpIdx]) icon = `<img style="width:18px;transform:translateY(20%);" src="${CARD_SUBTYPE_URL[sbtpIdx]}"/>`;
            else if (wpIdx > -1) icon = `<img style="width:18px;transform:translateY(20%);" src="${WEAPON_TYPE_URL[wpIdx]}"/>`;
        ...
        .replace(/(?<!\\)(\*?)\[(.*?)\]/g, (_, isUnderline, ctt) => {
            ...
            return `${wrapedIcon(el, el < 8 && ctt.includes('骰'))}<span style="color:${c};${isUnderline == '' ? `border-bottom:2px solid ${c};cursor:pointer;` : ''}margin-right:2px;${[-1, 8, 10].includes(el) ? 'margin-left:2px;' : ''}">${ctt}</span>`;
        }).replace(/\\/g, '');
    if (obj) {
        ...
        if ('shield' in obj) { // Summonee
            res = res.replace('{shield}', `${Math.abs(obj.shield)}`)
