<template>
  <div class="info-outer-container" @click="cancel()">
    ...
    >
      <div
        v-if="type == 5"
        @click.stop="showRule((info as Card).description, ...skillExplain.flat(2))"
      >
        ...
        <div>
          <div class="info-card-cost">
            <img
              class="cost-img"
              :src="getDiceIcon(ELEMENT_ICON[(info as Card).costType])"
            />
        ...
        <div
          class="info-card-type sub"
          v-for="(subtype, suidx) in (info as Card).subType.filter((v: number) => v > -1)"
          :key="suidx"
        >
          {{ CARD_SUBTYPE[subtype] }}
        </div>
        <div
          v-if="(info as Card).subType.includes(0)"
          class="info-card-type sub"
        >
        ...
        <div
          ...
          v-for="(desc, didx) in (info as Card).descriptions"
          :key="didx"
          v-html="desc"
        ></div>
        <div
          class="info-card-explain"
          v-for="(expl, eidx) in skillExplain"
          :key="eidx"
      ...
      <div v-if="type < 5">
        ...
        >
          <div
            class="info-hero-skill-title"
            @click.stop="showDesc(isShowSkill, sidx)"
          >
            <div
              ...
              <span class="info-skill-costs">
                ...
                <div>
                  ...
                  >
                    <img
                      class="cost-img"
                      :src="getDiceIcon(ELEMENT_ICON[cost.color])"
                    />
                    <span>{{
                      Math.max(
                        cost.val - (cidx < 2 ? skill.costChange[cidx] : 0),
                        0
                      )
                    }}</span>
                  </div>
          ...
          <div
            class="info-hero-skill-desc"
            v-if="isShowSkill[sidx]"
            @click.stop="
              showRule(
                skill.description,
                ...skillExplain[type < 4 ? type : sidx].flat(2)
              )
            "
          >
            <div class="skill-type">{{ SKILL_TYPE[skill.type] }}</div>
            <div
              v-for="(desc, didx) in skill.descriptions"
              :key="didx"
              v-html="desc"
            ></div>
          </div>
          <div
            v-if="
              isShowSkill[sidx] &&
              skillExplain[type < 4 ? type : sidx].length > 0
            "
            @click.stop="
              showRule(
                skill.description,
          ...
          >
            <div
              class="info-hero-skill-explain"
              v-for="(expl, eidx) in skillExplain[type < 4 ? type : sidx]"
              :key="eidx"
            >
              <div v-for="(desc, didx) in expl" :key="didx" v-html="desc"></div>
            </div>
        ...
        <div v-if="type != 4">
          ...
          >
            ...
            >
              <div
                ...
                @click.stop="showDesc(isEquipment, slidx)"
              >
              ...
              <div
                v-if="isEquipment[slidx]"
                @click.stop="showRule((slot as Card).description, ...slotExplain[slidx].flat(2))"
              >
                <div
                  class="equipment-desc"
                  v-for="(desc, didx) in (slot as Card).descriptions"
                  :key="didx"
                  v-html="desc"
                ></div>
                <div
                  class="info-card-explain"
                  v-for="(expl, eidx) in slotExplain[slidx]"
                  :key="eidx"
                >
                  <div
                    v-for="(desc, didx) in expl"
                    :key="didx"
                    v-html="desc"
                  ></div>
                  {{ slotExplain }}
          ...
          <div v-if="(info as Hero).inStatus.length > 0" class="info-status">
            ...
            <div
              v-for="(ist, idx) in (info as Hero).inStatus.filter((sts: Status) => !sts.type.includes(0))"
              :key="ist.id"
              class="status"
            >
                ...
                <span class="status-title-left">
                  <div class="status-icon">
                    ...
                    <img
                      ...
                      :style="{
                        filter:
                          ist.icon.startsWith('https') ||
                          ist.icon.startsWith('buff') ||
                          ist.icon.endsWith('dice')
                            ? `url(${getSvgIcon(
                                'filter'
                              )}#status-color-${STATUS_BG_COLOR.indexOf(
                                ist.iconBg
                              )})`
                            : '',
                      }"
                    ...
                    <div
                      class="status-cnt"
                      v-if="
                        !ist.type.includes(10) &&
                        (ist.useCnt >= 0 || ist.roundCnt >= 0)
                      "
                    >
                      {{ ist.useCnt < 0 ? ist.roundCnt : ist.useCnt }}
                    </div>
                  </div>
                  <span>{{ ist.name }}</span>
                </span>
              ...
              <div
                v-if="isInStatus[idx]"
                @click.stop="
                  showRule(ist.description, ...inStatusExplain[idx].flat(2))
                "
              >
                <div
                  class="status-desc"
                  v-for="(desc, didx) in ist.descriptions"
                  :key="didx"
                  v-html="desc"
                ></div>
                <div v-if="inStatusExplain[idx].length > 0">
                  <div
                    class="info-hero-status-explain"
                    v-for="(expl, eidx) in inStatusExplain[idx]"
                    :key="eidx"
                  >
                    <div
                      v-for="(desc, didx) in expl"
          ...
          <div v-if="(info as Hero).outStatus.length > 0" class="info-status">
            ...
            <div
              v-for="(ost, idx) in (info as Hero).outStatus.filter((sts: Status) => !sts.type.includes(0))"
              :key="ost.id"
              class="status"
            >
              <div
                class="status-title"
                @click.stop="showDesc(isOutStatus, idx)"
              >
                <span class="status-title-left">
                  <div class="status-icon">
                    <div
                    ...
                    <img
                      ...
                      :style="{
                        filter:
                          ost.icon.startsWith('https') ||
                          ost.icon.startsWith('buff') ||
                          ost.icon.endsWith('dice')
                            ? `url(${getSvgIcon(
                                'filter'
                              )}#status-color-${STATUS_BG_COLOR.indexOf(
                                ost.iconBg
                    ...
                    <div
                      class="status-cnt"
                      v-if="
                        !ost.type.includes(10) &&
                        (ost.useCnt >= 0 || ost.roundCnt >= 0)
                      "
                    >
                      {{ ost.useCnt < 0 ? ost.roundCnt : ost.useCnt }}
                    </div>
                  </div>
                  <span>{{ ost.name }}</span>
                </span>
              ...
              <div
                v-if="isOutStatus[idx]"
                @click.stop="
                  showRule(ost.description, ...outStatusExplain[idx].flat(2))
                "
              >
                <div
                  class="status-desc"
                  v-for="(desc, didx) in ost.descriptions"
                  :key="didx"
                  v-html="desc"
                ></div>
                <div v-if="outStatusExplain[idx].length > 0">
                  <div
                    class="info-hero-status-explain"
                    v-for="(expl, eidx) in outStatusExplain[idx]"
                    :key="eidx"
                  >
                    <div
                      v-for="(desc, didx) in expl"
      ...
      <div
        v-if="type == 6"
        @click.stop="showRule((info as Summonee).description)"
      >
        <div class="name">{{ (info as Summonee).name }}</div>
        <div
          class="summon-desc"
          v-for="(desc, didx) in (info as Summonee).descriptions"
          :key="didx"
          v-html="desc"
        ></div>
      </div>
    ...
    <div
      class="info-container info-rule"
      :class="{ 'mobile-font': isMobile }"
    ...
    >
      ...
      <div
        class="rule-desc"
        v-for="(rule, ridx) in ruleExplain"
        :key="ridx"
        v-html="rule"
      ></div>
    </div>
...
const wrapDesc = (desc: string, obj?: Skill | Status | Summonee): string => {
  let res = desc
    ...
    .replace(
      /(?<!\\)‹(\d+)(.*?)›/g,
      (_, c, v) =>
        `${wrapedIcon(c)}<span style="color:${ELEMENT_COLOR[c]};">${v}</span>`
    )
    .replace(/(?<!\\)(\*?)\[(.*?)\]/g, (_, isUnderline, ctt) => {
      const el = ELEMENT.findIndex(
        (v, vi) =>
  ...
  if (obj) {
    if ("dmgChange" in obj) {
      ...
      res = res.replace(
        /{dmg\+?\d*}/g,
        `${
          isChange ? `<span style="color:${CHANGE_GOOD_COLOR};">` : ""
        }${Math.abs(obj.damage + obj.dmgChange + dmg)}${
          isChange ? "</span>" : ""
        }`
      );
    }
    ...
    if ("shield" in obj) {
      // Summonee
      res = res
        .replace("{shield}", `${Math.abs(obj.shield)}`)
        .replace("{heal}", `${obj.shield}`);
    }
  }
  return res;
};
...
const wrapExpl = (expls: ExplainContent[]): string[][] => {
  ...
  for (const expl of expls) {
    ...
    explains.push(
      `<span style="font-weight:bold;color:white;">${expl.name}</span>`
    );
    explains.push(
      ...expl.description
        .split("；")
        .map((desc: string) => wrapDesc(desc, expl))
    );
...
const wrapRule = (...desc: string[]) => {
  ...
  [
    ...
    if (title in RULE_EXPLAIN) {
      ruleExplain.value.push(
        `<div style="font-weight:bold;border-top: 2px solid #6f84a0;padding-top:5px;">${wrapDesc(
          `*[${title}]`
        )}</div>`
      );
      ruleExplain.value.push(
        ...RULE_EXPLAIN[title].split("；").map((desc) => wrapDesc(desc))
      );
    }
  });
};
...
watchEffect(() => {
  ...
  if (info.value && "inStatus" in info.value) {
    ...
    info.value.inStatus.forEach((ist) => {
      ist.descriptions = ist.description
        .split("；")
        .map((desc) => wrapDesc(desc, ist));
      inStatusExplain.value.push(wrapExpl(ist.explains));
    });
    info.value.outStatus.forEach((ost) => {
      ost.descriptions = ost.description
        .split("；")
        .map((desc) => wrapDesc(desc, ost));
      outStatusExplain.value.push(wrapExpl(ost.explains));
    });
    slotExplain.value = [];
    [
      info.value.weaponSlot,
      info.value.artifactSlot,
      info.value.talentSlot,
    ].forEach((slot) => {
      if (slot != null) {
  ...
  if (info.value && "isDestroy" in info.value) {
    info.value.descriptions = (info.value.description as string)
      .split("；")
      .map((desc) => wrapDesc(desc, info.value as Summonee));
  }
});
