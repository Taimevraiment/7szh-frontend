...
export default class GeniusInvokationClient {
    ...
    checkCard() {
        ...
        this.player.heros.forEach((hero, i) => {
        ...
        if (userType == 0 && canSelectHero == heros.filter(h => h.isSelected).length ||
            ...
            const { willHeals } = this._doCmds([...(cardres.cmds ?? [])], { isCard: true, isOnlyRead: true, isExec: false });
    ...
    useCard() {
        ...
        this.player.playerInfo.destroyedSite += oSiteCnt - player.site.length;
    ...
    getPlayer(data: any) {
        ...
        if (data.isFlag) {
            this.socket.emit('sendToServer');
            if (data.isStart != undefined) this.isStart = data.isStart;
        }
    ...
    async getServerInfo(data: any) {
        ...
        if (!resetOnly && this.phase > 1 && this.isWin == -1) {
            ...
            if (players[this.playerIdx]?.phase == PHASE.ACTION && players[this.playerIdx ^ 1]?.phase == PHASE.ACTION_END &&
                ...
                setTimeout(async () => {
                    await this._wait(() => !this.taskQueue.isExecuting && this.taskQueue.isTaskEmpty(), { delay: 10 });
        ...
        if (this.phase == PHASE.DICE && phase == PHASE.ACTION_START) { // 开始阶段
            ...
            setTimeout(async () => {
                ...
                if (this.playerIdx == execIdx) {
                    await this._wait(() => this.isReseted, { delay: 10 });
    ...
    async useSkill(sidx: number, options: { isOnlyRead?: boolean, isCard?: boolean, isSwitch?: number, isReadySkill?: boolean, otriggers?: Trigger | Trigger[] }) {
        ...
        let { heros: oeHeros, summon: oeSummon, hidx: eFrontIdx } = this.opponent;
        const { heros: oaHeros, summon: oaSummon, hidx: ahidx } = this.player;
        ...
        if (sidx == -1 || isSwitchSelf(skillcmds)) {
            ...
            if (statusIdsAndPidx.length > 0) isSwitchAtk = true;
            doAfterStatus(aInStatusOppoPre[ahidx], 0, triggers, aswhidx, eswhidx, 0, true);
            doAfterStatus(aOutStatusOppoPre, 1, triggers, aswhidx, eswhidx, 0, true);
        }
        if (!isExec) {
                        ...
                        if (!isExecTask) {
                            ...
                            const siteHandle = [
                                ...
                                () => { // 数量、效果变化
                                    ...
                                    const { ndices, heros, willHeals } = this._doCmds(siteexecres?.cmds, { pidx });
                                    this._doHeal(willHeals, p.heros, { pidx });
                                    this.socket.emit('sendToServer', {
                                        cpidx: pidx,
                                        heros,
    ...
    */
    _doStatus(pidx: number, otypes: number | number[], otrigger: Trigger | Trigger[],
        ...
        const { intvl, isExec = true, isOnlyFront = false, players = this.players, phase = this.player.phase, dmgElement = 0,
            hidxs, hidx: ophidx = -1, card, isOnlyInStatus = false, isOnlyOutStatus = false, heal = [0, 0, 0],
    ...
    */
    ...
    _doSlot(otriggers: Trigger | Trigger[], options: {
        ...
        const { pidx = this.playerIdx, summons = this.players[pidx].summon, heal = [0, 0, 0], hcard, ehidx = -1,
            ...
            isChargedAtk = false, isFallAtk = this.isFall, isExec = true, card, intvl = [0, 0, 0, 0],
                    ...
                    for (const trigger of triggers) {
                        ...
                        if (isExec) {
                            if (slotres?.execmds) {
                                ...
                                const useSkillIdx = slotres.execmds.findIndex(v => v.cmd == 'useSkill');
                                if (useSkillIdx > -1) slotres.execmds.splice(useSkillIdx, 1);
                            }
                            if (taskMark) {
                            }
                            if (!taskMark) {
                                ...
                                const slotHandle = [
                                    ...
                                    (isEndAtk = false) => {
                                        ...
                                        const { ndices, heros: nh, inStatus: nist, willHeals } = this._doCmds(slotres.execmds, { pidx, heros: nheros });
                                        if (nh) nheros = nh;
                                        if (nist) nheros[hidx].inStatus = nist;
                                        if (slotexecres?.inStatus) {
                                            nheros[hidx].inStatus = this._updateStatus(slotexecres?.inStatus, nheros[hidx].inStatus).nstatus;
                                        }
                                        if (slotexecres?.outStatus) {
                                            nheros[hidx].outStatus = this._updateStatus(slotexecres?.outStatus, nheros[hidx].outStatus).nstatus;
                                        }
                                        this._doHeal(willHeals, nheros, { pidx });
                                        this.socket.emit('sendToServer', {
                                            cpidx: pidx,
                                            slotres: { cmds: slotres.execmds, slotIds: [hidx, slot] },
                                            heros: nheros,
     ...
     *                isExec 是否要去掉已选择的骰子, heros 角色组, eheros 敌方角色组
    ...
    _doCmds(cmds?: Cmds[],
        options: {
            ...
            heal?: number, isExec?: boolean, heros?: Hero[], eheros?: Hero[],
        } = {}
    ) {
        const { isCard = false, hidxs: chidxs, pidx = this.playerIdx,
            isOnlyRead = false, heal = 0, isExec = true, heros: ocheros, eheros: oceheros } = options;
        const player = this.players[pidx];
        const opponent = this.players[pidx ^ 1];
        ...
        for (let i = 0; i < cmds.length; ++i) {
            ...
            if (cmd == 'useSkill') {
                ...
                outStatusOppo = nstatus;
                if (nheros) eheros = nheros;
                if (eheros) eheros[fhidx].outStatus = [...nstatus];
        ...
        return { cmds, ndices, phase, heros, inStatus, outStatus, willHeals, changedEl, isSwitch, isSwitchOppo, inStatusOppo, outStatusOppo }
    }
    /**
    * 计算变化的伤害和骰子消耗
    * @param pidx 玩家识别符: 0对方 1我方
    ...
    _execTask(isWait = false) {
        ...
        return new Promise<void>(async resolve => {
            ...
            while (this._hasNotDieChange() && !this.taskQueue.isTaskEmpty() && this.taskQueue.isExecuting) {
                await this._wait(() => this.actionInfo == '', { delay: 10, freq: 100, isImmediate: false });
                ...
                if (isWait) {
                    ...
                    await this._wait(() => this._hasNotDieChange(), { delay: 10, recnt: 100000 });
                }
            }
