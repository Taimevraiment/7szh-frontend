...
export default class GeniusInvokationClient {
    ...
    useCard() {
        ...
        if (isInvalid) {
            ...
            const { ndices, phase = player.phase, willHeals, isSwitch = -1 } = this._doCmds(cardcmds, { isCard: true, heros: aHeros, isEffectHero: true, isRollDice: true, hidxs: isCdt(hidxs.length > 0, hidxs) });
            ...
            const cardrescmds: Cmds[] = [
                { cmd: 'getStatus', status: [...(cardres.inStatus ?? []), ...(cardres.outStatus ?? [])], hidxs: cardres.hidxs ?? hidxs },
                { cmd: 'getStatus', status: [...(cardres.inStatusOppo ?? []), ...(cardres.outStatusOppo ?? [])], hidxs: cardres.hidxs ?? hidxs, isOppo: true },
            ];
    ...
    async getServerInfo(data: any) {
        ...
        if (round > 1 || this.player?.phase != PHASE.CHOOSE_HERO || changeTo == this.playerIdx) {
            if (this.players.some(p => p.heros.some(h => {
                return [...h.inStatus, ...h.outStatus].some(sts => (sts.useCnt == 0 || sts.roundCnt == 0) && sts.type.every(t => ![1, 9, 15].includes(t)));
            }))) {
            ...
            setTimeout(async () => {
                ...
                if (this.round == 1) { // 游戏开始时
                    for (let hi = 0; hi < this.player.heros.length; ++hi) {
                        const { cmds: skillcmds = [] } = this._doSkill4(hi, 'game-start');
                        cmds.push(...skillcmds);
                    }
                }
                this.socket.emit('sendToServer', {
        ...
        if (actionAfter?.[0] == this.playerIdx) { // 我方执行任意行动后
            ...
            this.player.heros.forEach((_, hi) => isTriggered ||= this._doSkill4(hi, 'action-after').isTriggered);
    ...
    chooseHero() {
        ...
        if (this.player.phase == PHASE.CHOOSE_HERO) { // 选择初始出战角色
            ...
            isQuickAction = this._doSkill4(this.player.hidx, 'change-from', { isExec: false }).isQuickAction;
    ...
    reconcile(bool: boolean) {
        ...
        if (bool) {
            if (!this.isReconcile) {
                ...
                this._doStatus(this.playerIdx, 4, 'reconcile', { discards: this.handCards.filter(c => c.selected), isQuickAction: 2 });
    ...
    async useSkill(sidx: number, options: { isOnlyRead?: boolean, isCard?: boolean, isSwitch?: number, isReadySkill?: boolean, otriggers?: Trigger | Trigger[] } = {}) {
        ...
        if (sidx > -1) {
            ...
            this._doCmds(stsprecmds, { heros: aHeros, eheros: eaHeros, ahidx: hidx, ehidx: eFrontIdx, isEffectHero: true, isSkill: sidx });
        ...
        const calcAtk = (res: any, type: string, stsId: number, ahidx: number, ehidx: number, isSelf = false) => {
            ...
            const { isSwitch: csw = -1, isSwitchOppo: cswo = -1 } = this._doCmds(elrcmds3[0], { isExec: false, isSkill: sidx });
        ...
        const { ndices, isSwitch: swc = -1, isSwitchOppo: swco = -1 } = this._doCmds(skillcmds, { heros: aHeros, eheros: eaHeros, isEffectHero: true, isRollDice: true, isExec, isSkill: sidx });
        const { ndices: edices } = this._doCmds(eskillcmds, { heros: eaHeros, eheros: aHeros, isEffectHero: true, pidx: this.playerIdx ^ 1, isRollDice: true, isExec, isSkill: sidx });
        ...
        if (!isOnlyRead && currSkill.name == skill.name || isReadySkill) {
            ...
            const hasSkill5Task = this.player.heros.some((_, hi) => this._doSkill4(hi, 'action-after', { isExec: false }).isTriggered);
            this.socket.emit('sendToServer', {
                dices: ndices,
                edices,
                currSkill: skill,
                ...
                site: this.player.site,
                playerInfo: this.player.playerInfo,
                isEndAtk: ebHeros.length == 1 && !isSwitchAtk && this.taskQueue.isTaskEmpty() && !hasSkill5Task,
    ...
    changeHero() {
        ...
        if (dieChangeBack) {
            ...
            isQuickAction = this._doSkill4(this.player.hidx, 'change-from').isQuickAction;
    ...
    _elementReaction(
            ...
            if (attachElements[frontIdx].length == 0 || attachElements[frontIdx].includes(willAttach)) { // 没有元素反应
                ...
                if (willAttach == 5 && attachElement < 5) { // 扩散
                    ...
                    if (hasEls(4, 7)) { // 冰草共存
                        ...
                        res.willDamage[getDmgIdx][0] += 2;
                        if (efhero.isFront) res.elrcmds[0].push({ cmd: 'switch-after', cnt: 2500, isOppo: true });
        ...
        res.aheros.forEach((h, hi) => {
            doStatus(h.inStatus, true, res.atriggers[hi], hi);
            if (hi == aFrontIdx) doStatus(h.outStatus, true, res.atriggers[hi], hi);
        });
        res.eheros.forEach((h, hi) => {
            doStatus(h.inStatus, false, res.etriggers[hi], hi);
            if (hi == frontIdx) doStatus(h.outStatus, false, res.etriggers[hi], hi);
        });
        if (!isWind) {
        ...
        this._doSkill4(frontIdx, etriggers[frontIdx], { pidx: pidx ^ 1, heros: res.eheros, isExec, getdmg: restDmg });
                        ...
                        const summonHandle = [
                            ...
                            () => { // 扣血、显示伤害、效果变化
                                ...
                                if (smnexecres?.cmds) {
                                    ...
                                    const aWillAttachs: number[][] = new Array(aheros.length + eheros.length).fill(0).map(() => []);
                                    let aElTips: [string, number, number][] = new Array(aheros.length + eheros.length).fill(0).map(() => ['', 0, 0]);
                                    const { changedEl, inStatus, outStatus, heros, willHeals, ndices } = this._doCmds(smnexecres.cmds, { pidx, heal: summon.shield, isRollDice: true });
                                    if (changedEl) summon.element = changedEl;
                                    ...
                                    for (let i = 0; i < smnexecres.cmds.length; ++i) {
                                        ...
                                        if (cmd == 'attack') {
                                            ...
                                            let { willDamage: willDamage1, willAttachs, dmgElements: dmgElements1, eheros: eheros1, esummon: esummon1,
                                            ...
                                            aSummon = [...(isOppo ? esummon1 : asummon1)];
                                            aElTips = [...elTips1];
                                            for (let i = 0; i < aheros.length; ++i) aWillAttachs[i + (pidx ^ +isOppo) * (isOppo ? aheros : eheros).length].push(willAttachs[i]);
                                        ...
                                        if (isAttach) {
                                            ...
                                            aElTips = [...elTips2];
                                            for (let i = 0; i < aheros.length; ++i) aWillAttachs[i + pidx * eheros.length].push(willAttachs[i]);
                                        }
                                        currSummon = summon;
                                    }
    ...
    _doSite(pidx: number, ostates: Trigger | Trigger[],
        ...
        let task: [(() => void)[], number[]] | undefined;
        if (players.some(p => p.hidx < 0)) return { isQuickAction, cmds, exchangeSite, minusDiceHero, siteCnt, task }
        const p = players[pidx];
                        ...
                        if (!isExecTask) {
                            ...
                            const siteHandle = [
                                ...
                                () => { // 数量、效果变化
                                    ...
                                    const { ndices, heros: nh, eheros, willHeals } = this._doCmds(siteexecres.cmds, { pidx, isRollDice: true });
    ...
    */
            ...
            for (const sts of stses) {
                ...
                if (types.includes(1) && (stsres.damage || stsres.pendamage || stsres.heal)) {
                    statusIdsAndPidx.push({
                        id: sts.id, type: group, pidx, isSelf: +!!stsres.isSelf, trigger, hidx, isSwitchAtk,
                        isQuickAction: isQuickAction == 2 || (!!card && !card.subType.includes(7)) || trigger.startsWith('action-start'),
                    });
                }
                if (isExec) {
                    ...
                    if (stsres.isAddTask || (intvl && (!types.includes(1) || !sts.type.includes(1)) && !stsres.damage && !stsres.pendamage && !stsres.heal)) {
                        const tintvl = clone(intvl) ?? [100, 100, 100, 100];
                        ...
                        if (!taskMark) {
                            ...
                            const statusHandle = [
                                ...
                                () => { // 回血
                                    ...
                                    // let willDamage: number[][] | undefined;
                                    // let elTips: [string, number, number][] | undefined;
                                    // let isSelf: boolean = stsres.isSelf ?? false;
                                    // let aWillAttach: number[][] | undefined;
                                    // let aWillHeal: number[] = new Array(aheros.length + eheros.length).fill(-1);
                                    // let willAttachs: number[][] | undefined;
                                    // let dmgElements: number[] = new Array(eheros.length).fill(0);
                                    const { heros, ndices, willHeals } = this._doCmds(stscmds, { pidx, hidxs: [hidx], isRollDice: true });
                                    if (heros) aheros = heros;
                                    ...
                                    //     const eFrontIdx = isSelf ? hidx : eheros.findIndex(h => h.isFront);
                                    //     const apidx = pidx ^ +!isSelf;
                                    ...
                                    //     asummon = isSelf ? esummon1 : asummon1;
                                    //     esummon = isSelf ? asummon1 : esummon1;
                                    //     dmgElements = [...dmgElements1];
                                    //     aWillAttach = new Array(aheros.length + eheros.length).fill(0).map(() => []);
                                    //     for (let i = 0; i < aheros.length; ++i) aWillAttach[i + (apidx ^ 1) * eheros.length].push(willAttachs1[i]);
                                    //     willDamage = [...willDamage1];
                                    //     stscmds.push(...elrcmds1[0]);
                                    //     elTips = elTips1;
                                    //     willAttachs = new Array(aheros.length + eheros.length).fill(0).map(() => []);
                                    //     willAttachs[eFrontIdx + (pidx ^ +!isSelf) * ((isSelf ? eheros : aheros).length)].push(stsres.element ?? 0);
                                    // }
                                    // if (stsres.heal) {
                                    //     const stshl = stsres.heal ?? 0;
                                    //     (stsres.hidxs ?? [isSelf ? eheros.findIndex(h => h.isFront) : hidx]).forEach(hli => {
                                    //         const hlhidx = hli + (pidx ^ +!isSelf) * ((isSelf ? eheros : aheros).length);
                                    ...
                                    this.socket.emit('sendToServer', {
                                        ...
                                        dices: ndices,
                                        playerInfo: this.players[pidx].playerInfo,
                                        isEndAtk: this.taskQueue.isTaskEmpty() && (curStatus.type.includes(13) || ['reconcile', 'discard'].includes(trigger)),
                                        isQuickAction: isQuickAction == 2 || (!!card && !card.subType.includes(7)) || trigger.startsWith('action-start'),
                                        step: 1,
    ...
    _doStatusAtk(stsTask: StatusTask) {
        return new Promise<boolean>(async resolve => {
            ...
            for (let i = 0; i < aheros.length; ++i) aWillAttach[i + (pidx ^ 1) * eheros.length].push(willAttachs1[i]);
            let willHeals: number[] | undefined;
            const cmds = [...(stsres.cmds ?? []), ...(execmds ?? [])];
            let dices;
            if (cmds.length > 0) {
                const { willHeals: cmdheal, ndices, heros } = this._doCmds(cmds, { pidx, heros: aheros, isRollDice: true });
                willHeals = cmdheal;
                dices = ndices;
                if (heros) aheros = heros;
            }
            ...
            willAttachs[eFrontIdx + (pidx ^ 1) * aheros.length].push(stsres.element ?? 0);
            this.socket.emit('sendToServer', {
    ...
    * @param trigger 触发的时机
    * @param options pidx 玩家idx, players 玩家组, heros 角色组, eheros 敌方角色组, isExec 是否执行, getdmg 受到伤害数, isEffectHero 是否直接作用于改变角色组, heal 回血数
    ...
    */
    _doSkill4(hidx: number, otrigger: Trigger | Trigger[],
        options: {
            ...
            isExec?: boolean, getdmg?: number, isEffectHero?: boolean, heal?: number[],
        } = {}
    ) {
        if (hidx < 0) return { isQuickAction: false, heros: [], eheros: [], isTriggered: false }
        const { pidx = this.playerIdx, players = this.players, isExec = true, getdmg = 0, isEffectHero = true, heal = [] } = options;
        let { heros = players[pidx].heros, eheros = players[pidx ^ 1].heros } = options;
        let isTriggered = false;
        const hero = heros[hidx];
        let isQuickAction = false;
        const skills = herosTotal(hero.id).skills;
        const skill4s = skills.filter(sk => sk.type == 4);
        const triggers: Trigger[] = [];
        if (typeof otrigger == 'string') triggers.push(otrigger);
        else triggers.push(...otrigger);
        const cmds: Cmds[] = [];
        for (const skill4 of skill4s) {
            for (const trigger of triggers) {
                const skidx = skills.findIndex(sk => sk.type == 4 && sk.name == skill4.name);
                const skillres = skill4.handle({ hero, skidx, getdmg, trigger, heros, heal });
                if (!skillres.trigger?.includes(trigger)) continue;
                isTriggered = true;
                cmds.push(...(skillres.cmds ?? []));
                cmds.push({ cmd: 'getStatus', status: [...(skillres.inStatus ?? []), ...(skillres.outStatus ?? [])], hidxs: [hidx] });
        ...
        if (hidx == heros.findIndex(h => h.isFront)) {
            for (let i = 0; i < eheros.length; ++i) {
                ...
                const eskill4s = eskills.filter(sk => sk.type == 4);
                for (const eskill4 of eskill4s) {
                    const skilloppores = eskill4.handle({ hero: ehero, skidx: eskills.findIndex(sk => sk.name == eskill4.name) });
                    for (const trg of triggers) {
                        if (!skilloppores.trigger?.includes(trg)) continue;
                        if (skilloppores.outStatusOppo) cmds.push({ cmd: 'getStatus', status: skilloppores.outStatusOppo });
                    }
                }
            }
        }
        ...
        if (nh) heros = nh;
        return { isQuickAction, heros, eheros, cmds, isTriggered }
    }
    /**
     * 检测装备栏
     * @param triggers 触发时机
    ...
    _doSlot(otriggers: Trigger | Trigger[], options: {
        ...
        for (const hidx of hidxs) {
            ...
            //                     const isOnlyEffect = cmds[i][0].cmd == '';
            //                     if (!isOnlyEffect) {
            //                         const { ndices: nd, willHeals } = this._doCmds(cmds[i], { pidx, heros, eheros, isEffectHero: true, isRollDice: true });
            //                         dices = nd;
            //                         this._doHeal(willHeals, heros, { pidx, isQuickAction });
            //                     }
            //                     this.socket.emit('sendToServer', {
     ...
     * 弃牌后的处理
     * @param cards 弃牌
     * @param options pidx 玩家idx, heros 角色组, isSkill 使用技能idx
     */
     ...
     *                ahidx 攻击角色hidx, ehidx 受击角色hidx, isEffectHero 是否直接作用于改变角色组, isRollDice 是否检测骰子, isSkill 使用技能的idx
     * @returns ndices 骰子, phase 阶段, heros 角色组, eheros 敌方角色组, inStatus 获得角色状态, willHeals 回血组, changedEl 变化元素
     */
    _doCmds(cmds?: Cmds[],
        options: {
            isCard?: boolean, hidxs?: number[], pidx?: number, heal?: number, isRollDice?: boolean, isSkill?: number,
        ...
        const { isCard = false, hidxs: chidxs, pidx = this.playerIdx, heal = 0, isRollDice = false,
            isExec = true, heros: ocheros, eheros: oceheros, isEffectHero = false, isSkill = -1 } = options;
        const player = this.players[pidx];
        const opponent = this.players[pidx ^ 1];
        ...
        for (let i = 0; i < cmds.length; ++i) {
            ...
            if (cmd == 'useSkill') {
                ...
                if (isExec) this._doDiscard(discards, { pidx, heros: cheros, isSkill });
                ...
                (cmds[i].hidxs ?? []).forEach(hidx => {
                    const { heros: sk5h } = this._doSkill4(hidx, 'revive', { pidx, heros: heros ?? cheros, isEffectHero: false });
                    heros = sk5h;
                });
    ...
    */
    _calcSkillChange(pidx: number, hidx: number, options: { isUpdateHandcards?: boolean, isSwitch?: boolean, isReadySkill?: number } = {}) {
        ...
        if (isSwitch) this._doSkill4(hidx, 'change-to', { heros });
