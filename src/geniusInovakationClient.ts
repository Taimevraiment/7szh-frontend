...
export default class GeniusInvokationClient {
    ...
    useCard() {
        ...
        if (isInvalid) {
            ...
            let esummon: Summonee[] = this._updateSummon([], opponent.summon, opponent.heros[opponent.hidx].outStatus);
            ...
            this._doHeal(willHeals, aHeros, { isEndAtk: currCard.subType.includes(7) });
            ...
            let outStatusOppo: Status[] = opponent.heros[opponent.hidx].outStatus;
            if (cardres?.inStatus) {
            ...
            if (cardres?.outStatusOppo) {
                const { nstatus, nheros } = this._updateStatus(cardres?.outStatusOppo ?? [], outStatusOppo, opponent.heros, opponent.hidx);
                outStatusOppo = nstatus;
                if (nheros) {
                    opponent.heros[opponent.hidx] = nheros[opponent.hidx];
                    opponent.heros[opponent.hidx].outStatus = [...nstatus];
                }
            }
            player.heros = [...aHeros];
    ...
    async getServerInfo(data: any) {
        ...
        if (!resetOnly && this.phase > 1 && this.isWin == -1) {
            ...
            if (players[this.playerIdx ^ 1]?.phase == PHASE.ACTION && players[this.playerIdx]?.phase == PHASE.ACTION_END &&
                (isUseSkill || isChangeHero) && !this.isSwitchAtking) {
                setTimeout(async () => {
                    await this._wait(() => !this.taskQueue.isExecuting && this.taskQueue.isTaskEmpty(), { delay: 0 });
                    this._sendTip('对方继续回合');
                }, 2000);
            }
        ...
        if (actionStart == this.playerIdx && ![PHASE.DIE_CHANGE, PHASE.DIE_CHANGE_END].includes(players[this.playerIdx].phase)) { // 我方选择行动前
            ...
            this._doSummon(this.playerIdx, 'action-start', { players });
        ...
        const hasReadySkill = this.player?.heros?.[this.player?.hidx]?.inStatus.some(ist => ist.type.includes(11)) &&
            this.player?.heros?.[this.player?.hidx]?.inStatus.every(ist => !ist.type.includes(14));
        ...
        if (this.phase == PHASE.DICE && phase == PHASE.ACTION_START) { // 开始阶段
            this.showRerollBtn = true;
            this.isFall = false;
            this._sendTip(`第${this.round}轮开始`);
    ...
    async useSkill(sidx: number, options: { isOnlyRead?: boolean, isCard?: boolean, isSwitch?: number, isReadySkill?: boolean, otriggers?: Trigger | Trigger[] }) {
        ...
        if (sidx > -1) {
            ...
            const stsprecmds: Cmds[] = [
                ...
                { cmd: 'getInStatusOppo', status: skillres.outStatusOppoPre, hidxs: skillres.hidxs },
            ...
            const stscmds: Cmds[] = [
                ...
                { cmd: 'getInStatusOppo', status: skillres.outStatusOppo, hidxs: skillres.hidxs },
        ...
        const calcAtk = (res: any, type: string, stsId: number, ahidx: number, ehidx: number, isOppo = false) => {
            ...
            if (isSwitchOppo(elrcmds3[0])) {
                if (isOppo) {
                    ...
                    doAfterStatus(aHeros[ahidx].inStatus, 0, ['change-from'], ahidx, ehidx, 0, true);
                    doAfterStatus(aHeros[hidx].outStatus, 1, ['change-from'], ahidx, ehidx, 0, true);
                } else {
                    const { statusIdsAndPidx } = this._doStatus(this.playerIdx ^ 1, 1, 'change-from', { hidxs: [ehidx], isExec: false });
                    if (statusIdsAndPidx.length > 0) isSwitchAtk = true;
                    doAfterStatus(eaHeros[ehidx].inStatus, 0, ['change-from'], ahidx, ehidx, 1, true);
                    doAfterStatus(eaHeros[eFrontIdx].outStatus, 1, ['change-from'], ahidx, ehidx, 1, true);
                }
            }
            return false;
        }
        ...
        if (sidx > -1) {
            ...
            eaHeros.forEach((h, hi) => {
                ...
                if (hi == eFrontIdx) doAfterStatus(h.outStatus, 1, afterESkillTrgs[hi], aswhidx, eswhidx, 1);
            });
            aHeros.forEach((h, hi) => {
        ...
        if (isSwitchOppo(skillcmds)) {
            ...
            if (statusIdsAndPidx.length > 0) isSwitchAtk = true;
            doAfterStatus(eaHeros[eFrontIdx].inStatus, 0, ['change-from'], aswhidx, eswhidx, 1, true);
            doAfterStatus(eaHeros[eFrontIdx].outStatus, 1, ['change-from'], aswhidx, eswhidx, 1, true);
        }
     ...
     */
    _sendTip(content: string, top?: string, color?: string, time?: number) {
                        ...
                        const summonHandle = [
                            ...
                            () => { // 扣血、显示伤害、效果变化
                                ...
                                if (smnexecres?.cmds) {
                                    ...
                                    const { changedEl, inStatus, outStatus, heros, willHeals } = this._doCmds(smnexecres.cmds, { pidx, heal: summon.shield });
                                    if (changedEl) summon.element = changedEl;
                                    ...
                                    for (let i = 0; i < smnexecres.cmds.length; ++i) {
                                        ...
                                        if (cmd == 'attack') {
                                            ...
                                            aSummon = [...asummon1];
                                            aElTips = [...elTips1];
                                            for (let i = 0; i < 3; ++i) aWillAttachs[i + (pidx ^ 1) * 3].push(willAttachs[i]);
                                                ...
                                                [...eheros[eFrontIdx].inStatus, ...eheros[eFrontIdx].outStatus].forEach(sts => {
                                                    if (sts.type.includes(1)) {
                                                        ...
                                                        if (stsres.trigger?.includes(trigger) && (stsres.damage || stsres.pendamage || stsres.heal)) {
                                                            ...
                                                            statusIdsAndPidx.push({ id: sts.id, type: sts.group, pidx: pidx ^ isOppo ^ 1, isOppo, trigger });
                                                        }
                                                    }
                                                });
                                            }
                            ...
                            (isEndAtk = false) => { // 更新summon数据
                                const outStatus = this.players[pidx].heros[p.hidx].outStatus;
                        ...
                        if (!isExecTask) {
                            ...
                            const siteHandle = [
                                ...
                                () => { // 数量、效果变化
                                    ...
                                    const { ndices, heros, eheros, willHeals } = this._doCmds(siteexecres.cmds, { pidx });
                                    this._doHeal(willHeals, p.heros, { pidx });
                                    this.socket.emit('sendToServer', {
                                        cpidx: pidx,
                                        heros,
    ...
    _doStatus(pidx: number, otypes: number | number[], otrigger: Trigger | Trigger[],
        ...
        const doStatus = (stses: Status[], group: number, hidx: number, trigger: Trigger) => {
            ...
            for (const sts of stses) {
                ...
                if (sts.type.includes(1) && (stsres.damage || stsres.pendamage || stsres.heal) && (stsres.cmds?.length ?? 0) == 0) {
                ...
                if (isExec) {
                    ...
                    if (stscmds.length > 0) {
                        ...
                        if (intvl) {
                            ...
                            const { ndices, heros } = this._doCmds(stscmds, { pidx });
                            if (heros) players[pidx].heros = heros;
                            this.socket.emit('sendToServer', {
                                cpidx: pidx,
                                heros: heros ?? pheros,
                                dices: ndices,
                                cmds: stsres.cmds,
                                flag: `_doStatus-${['in', 'out'][group]}Status-immediate-${sts.name}-${pidx}`,
                            });
                        }
                    }
    ...
    _doAddAtk(stsTask: StatusTask) {
        return new Promise<boolean>(async resolve => {
            ...
            const handleres = stype == 0 ?
                heroStatus(sid).handle(frontHero.inStatus
                ...
                heroStatus(sid).handle(frontHero.outStatus
            ...
            const handlereshidxs = handleres.hidxs ?? getBackHidxs(isOppo ? aheros : eheros);
            let { willDamage, willAttachs: willAttachs1, dmgElements, eheros: eheros1, summon: esummon, aheros: aheros1, summonOppo, elrcmds, elTips }
                = this._elementReaction(
                    handleres.element ?? 0,
                    new Array((isOppo ? aheros : eheros).length).fill(0).map((_, i) => [
                        i == atkedIdx ? (handleres.damage ?? -1) : -1,
                        (handlereshidxs.includes(i)) ? (handleres.pendamage ?? 0) : 0
                    ]),
                    atkedIdx,
                    isOppo ? aheros : eheros,
                    isOppo ? aSummon : eSummon,
            ...
            const { cmds: execmds = [] } = handleres.exec?.((stype == 0 ?
            ...
            if (handleres.heal) {
                const heals = new Array(aheros1.length).fill(0).map((_, hi) => {
                    return (handleres.hidxs ?? [ahidx]).includes(hi) ? (handleres.heal ?? 0) : -1;
                });
                ...
                for (let i = 0; i < 3; ++i) {
                    ...
                    willHeals[hlidx] = Math.min(aheros1[hlidx].maxhp - aheros1[hlidx].hp, willHeals[hlidx] + heals[i]);
                }
            }
            this._doHeal(willHeals, aheros1, { pidx });
            const statusIdsAndPidx: StatusTask[] = [];
            const triggers: Trigger[] = ['status-destroy'];
            ...
            this.socket.emit('sendToServer', {
                statusId: stype < 2 ? [sid, stype, isOppo, ahidx, isa, isQuickAction] : [],
                cpidx: pidx,
                summonee: isOppo ? esummon : summonOppo,
                heros: isOppo ? eheros1 : aheros1,
                eheros: isOppo ? aheros1 : eheros1,
                esummon: isOppo ? summonOppo : esummon,
    ...
    */
    _doSkill5(hidx: number, otrigger: Trigger | Trigger[],
        ...
        for (const skill5 of skill5s) {
            for (const trigger of triggers) {
                const skillres = skill5.handle({ hero, skidx: skills.findIndex(sk => sk.name == skill5.name), dmg, trigger, heros });
    ...
    _doSlot(otriggers: Trigger | Trigger[], options: {
        ...
        isExec?: boolean, intvl?: number[], dieChangeBack?: boolean, usedDice?: number, isEndAtk?: boolean, getdmg?: number[],
    } = {}) {
        const triggers: Trigger[] = [];
                        ...
                        const slotHandle = [
                            ...
                            (isEndAtk = false) => {
                                ...
                                if (!isOnlyEffect) {
                                    ...
                                    this._doHeal(willHeals, nheros, { pidx });
                                }
                                this.socket.emit('sendToServer', {
                                    cpidx: pidx,
        ...
        return {
            willHeals, pidx, changeHeroDiceCnt, addDmg, addDmgSummon, inStatus: isCdt(inStatus.length > 0, inStatus),
            outStatus: isCdt(outStatus.length > 0, outStatus), minusDiceHero, summon, isEndAtk: oisEndAtk, minusDiceCard, minusDiceSkill, task,
        };
    }
    /**
     * 检测回血
     * @param willHeal 回血量
     * @param heros 回血角色组
     * @param options pidx 玩家idx, isEndAtk 是否结束攻击, isExec 是否执行
     */
    _doHeal(willHeal: number[] | undefined, heros: Hero[], options: { pidx?: number, isExec?: boolean, isEndAtk?: boolean } = {}) {
        const { isExec = true, isEndAtk, pidx = this.playerIdx } = options;
        const heal = willHeal?.slice(3 - pidx * 3, 6 - pidx * 3).map(v => Math.max(0, v) % 100);
        if (heal?.some(hl => hl > 0)) {
            this._doSlot('heal', { pidx, hidxs: allHidxs(heros, { isAll: true }), heal, heros, isExec, isEndAtk });
            this._doStatus(pidx, 4, 'heal', { heal, isExec });
            const { siteCnt } = this._doSite(pidx, 'heal', { heal, isExec });
            this._doSite(pidx ^ 1, 'eheal', { heal, isExec });
     ...
     * 执行命令集
     * @param cmds 命令集
     * @param options isCard 是否为使用卡, hidxs 角色索引组, pidx 执行命令玩家idx, isOnlyRead 是否只读, heal 回血量,
     *                isExec 是否要去掉已选择的骰子, heros 角色组, eheros 敌方角色组, ahidx 攻击角色hidx, ehidx 受击角色hidx,
     *                isEffectHero 是否直接作用于改变角色组
     * @returns ndices 骰子, phase 阶段, heros 角色组, eheros 敌方角色组, inStatus 获得角色状态, willHeals 回血组, changedEl 变化元素
     */
    _doCmds(cmds?: Cmds[],
        options: {
            isCard?: boolean, hidxs?: number[], pidx?: number, isOnlyRead?: boolean, heal?: number,
            isExec?: boolean, heros?: Hero[], eheros?: Hero[], ahidx?: number, ehidx?: number, isEffectHero?: boolean
        } = {}
    ) {
        const { isCard = false, hidxs: chidxs, pidx = this.playerIdx, isOnlyRead = false, heal = 0,
        ...
        for (let i = 0; i < cmds.length; ++i) {
            ...
            if (cmd == 'useSkill') {
                this.useSkill(cnt, { isOnlyRead, isCard, isSwitch, isReadySkill });
                ...
                if (getstatus.length == 0) continue;
    ...
    */
        ...
        return new Promise<void>(async resolve => {
            ...
            while (this._hasNotDieChange() && !this.taskQueue.isTaskEmpty() && this.taskQueue.isExecuting) {
                ...
                if (taskType.startsWith('addAtk')) {
                    const isExeced = await this._doAddAtk(args as StatusTask);
                    if (!isExeced) {
                ...
                if (isWait) {
                    ...
                    await this._wait(() => this._hasNotDieChange(), { delay: 0, recnt: 100000 });
                }
            }
     ...
     */
    async _wait(cdt: () => boolean, options: { delay?: number, freq?: number, recnt?: number, isImmediate?: boolean } = {}) {
        const { delay = 2000, freq = 500, recnt = 50, isImmediate = true } = options;
        let loop = 0;
        ...
        while (true) {
            ...
            if (loop > recnt) throw new Error('too many loops: ' + cdt.toString());
        }
    }
}
