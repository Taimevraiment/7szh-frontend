<template>
  <div class="container">
      ...
      <div class="lobby">
        <div class="game-list-container">
          ...
          <div class="game-list">
            <div class="game-room" v-for="(room, ri) in roomList" :key="'roomList' + ri"
              @click="room.hasPassWord ? openEnterRoom(room.id) : enterRoom(room.id.toString(), '')">
        ...
        <div class="player-list-container">
          ...
          <div class="player-list">
            <div class="player-item" v-for="(player, pi) in playerList" :key="'playerList' + pi">
              <div class="player-name">{{ player.name }}</div>
              <div class="player-status">{{ player.rid < 0 ? '空闲' : '游戏中' }}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    <create-room-modal v-if="isShowCreateRoom" @create-room-cancel="cancelCreateRoom" @create-room="createRoom" />
    <enter-room-modal v-if="isShowEnterRoom" :select-room-id="selectRoomId" @enter-room-cancel="cancelEnterRoom"
...
// 加入房间
const enterRoom = (roomId: string, roomPassword: string) => {
  isShowEnterRoom.value = false;
  socket.emit('enterRoom', { roomId: Number(roomId), roomPassword });
}
...
// 获取玩家列表
socket.on('getPlayerList', ({ plist }) => playerList.value = (plist as PlayerList).filter(p => p.id != userid.value));
